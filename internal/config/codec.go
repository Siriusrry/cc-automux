...
func checkProfileKeys(object map[string]json.RawMessage) error {
	if err := rejectUnknownKeys(object, map[string]struct{}{
		...
		"teammate_default_model": {}, "max_attempts": {}, "sticky_no_cooldown_attempts": {},
	}); err != nil {
		return err
	}
	for key, value := range object {
		if key == "max_attempts" || key == "sticky_no_cooldown_attempts" {
			if _, err := DecodeAttemptLimit(key, value); err != nil {
				return err
			}
			continue
		}
...
// DecodeAttemptLimit shares strict, field-specific validation across APIs.
func DecodeAttemptLimit(field string, raw json.RawMessage) (int, error) {
	var value int
	if bytes.Equal(bytes.TrimSpace(raw), []byte("null")) {
		return 0, fmt.Errorf("%s must not be null", field)
	}
	if err := json.Unmarshal(raw, &value); err != nil {
		return 0, fmt.Errorf("%s must be an integer", field)
	}
	if err := ValidateAttemptLimit(field, value); err != nil {
		return 0, err
	}
	return value, nil
}

func ValidateAttemptLimit(field string, value int) error {
	if value < 1 || int64(value) > MaxSafeAttempts {
		return fmt.Errorf("%s must be an integer from 1 to 9007199254740991", field)
	}
	return nil
}
